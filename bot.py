...
async def today_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn = await get_db()
    rows = await conn.fetch("SELECT * FROM expenses WHERE date = $1 ORDER BY created_at", today)
    await conn.close()
    if not rows:
...
async def last_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    conn = await get_db()
    rows = await conn.fetch("SELECT * FROM expenses ORDER BY created_at DESC LIMIT 5")
    await conn.close()
    if not rows:
...
async def delete_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    conn = await get_db()
    row = await conn.fetchrow("SELECT * FROM expenses ORDER BY created_at DESC LIMIT 1")
    if not row:
