...
const save = d => fs.writeFileSync(DATA, JSON.stringify(d, null, 2), 'utf8');
...
Type /help for all commands.`
  ...
  if (tx.type === 'credit') {
    return say(cid,
`✅ <b>Income logged:</b> +${f2(tx.amount)} AED${tx.description ? ` — ${tx.description}` : ''}
💰 Total income this month: <b>${f2(c.income)} AED</b>`,
      KB_MAIN
    );
  }
...
const server = http.createServer((req, res) => {
  ...
  if (pathname.startsWith('/api/transaction/') && req.method === 'DELETE') {
  ...
  if (pathname.startsWith('/api/transaction/') && req.method === 'PUT') {
    const id = pathname.split('/').pop();
    let body = '';
    req.on('data', c => body += c);
    req.on('end', () => {
